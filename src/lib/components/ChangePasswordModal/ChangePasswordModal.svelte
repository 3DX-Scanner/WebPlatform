...
<div class="fixed inset-0 z-[1000] bg-black/40 flex items-center justify-center" role="dialog" aria-modal="true" tabindex="-1" onclick={handleBackdrop} onkeydown={handleKey}>
    <div class="bg-card rounded-2xl shadow-2xl w-full max-w-lg p-6">
        <div class="flex items-start justify-between mb-4">
            <h3 class="text-xl font-bold text-card-foreground">Changer le mot de passe</h3>
            <button class="text-muted-foreground hover:text-foreground" onclick={close}>✕</button>
        </div>
        ...
        <div class="mt-6 flex items-center justify-end gap-3">
            ...
            <Button variant="default" onclick={save} disabled={loading || !!error || !newPassword || !confirmPassword}>
