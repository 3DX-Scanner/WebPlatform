<script lang="ts">
    ...
    import { user, isAuthenticated, checkAuth } from '$lib/stores/auth';
    ...
    <div class="layout">
        <aside class="sidebar">
            <div class="card glass profile" bind:this={leftCardEl}>
                <div class="avatar-xl">{$user ? $user.email.charAt(0).toUpperCase() : '?'}</div>
                <div class="name">{$user?.name || 'Utilisateur'}</div>
                <div class="email">{$user?.email || 'non connecté'}</div>
                <div class="badge">Membre depuis {$user ? new Date($user.createdAt).toLocaleDateString('fr-FR') : '-'}</div>
